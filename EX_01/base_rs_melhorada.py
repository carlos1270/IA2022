...
rs = pd.read_csv('BaseRS_melhorada.csv', sep=",", header=0)
...
rs_gabarito = pd.read_csv('BaseRS_melhorada.csv', sep=",", header=0)
gabarito = []
...
sns.pairplot(rs, hue="target-kmeans", palette=['blue', 'red', 'green'])
pyt.show()
...
kmeans_centroides = pd.concat([rs, df_centroides])
...
""" 
    ...
    Melhorado, tem um score de 0,96
"""
sns.pairplot(kmeans_centroides, hue="target-kmeans", palette=['blue', 'red', 'green', 'purple', 'orange', 'yellow'])
pyt.show()